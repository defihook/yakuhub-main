...
import { Grid, CardContent, useTheme, Box, Typography, Chip, IconButton, Divider } from '@mui/material';
import { DeleteOutlined } from '@mui/icons-material';
import { capitalize, get, last } from 'lodash';

const EmployCard = ({ time: datetime, project, name, wallet, method, amount, period, employer, transactionHash, onDelete, index }: any) => {
        ...
        <>
            {project ? (
                <MainCard
                    ...
                    secondary={
                        <IconButton color="error" onClick={() => onDelete({ project, name, wallet, employer }, index)}>
                            <DeleteOutlined />
                        </IconButton>
                    }
                >
                    <CardContent sx={{ p: 0, pb: '0px !important' }}>
                        ...
                        <Grid item xs={12} sx={{ mb: '10px', mt: '5px' }}>
                            <Typography component="p">Next Payment: </Typography>
                        ...
                        <Grid item xs={6} md={1}>
                            {!transactionHash || !transactionHash.length ? (
                                ...
                                <a
                                    href={`https://solscan.io/tx/${get(last(transactionHash), 'txHash', '')}`}
                                    target="_blank"
                                    rel="noreferrer"
                                >
