                    ...
                    <CardContent sx={{ p: 2, pb: '16px !important' }}>
                        ...
                        <Box display="flex" justifyContent="space-between" sx={{ alignItems: 'center', minHeight: 44 }}>
                            ...
                            <Tooltip title="Employees">
                                ...
                                >
                                    {map(get(notifyProjects, ['employees', project], []), (employee: any = { name: '' }, idx) => (
                                        <Avatar
                                            key={idx}
                                            {...(employee.avatar
                                                ? { src: `${IMAGE_PROXY}${employee.avatar}` }
                        ...
                        <Grid container sx={{ alignItems: 'center' }}>
                            ...
                            >
                                <Chip label={`${balance.toFixed(2)} USDC`} size="small" color="info" />
                                <Chip label={`${solBalance.toFixed(2)} SOL`} size="small" color="secondary" />
                            </Grid>
                            ...
                            >
                                ...
                                {balance > 0 && solBalance <= 0.001 && (
                                    <Tooltip title="Insufficient transaction fee">
                                        <Chip
                                            sx={{ mr: 1 }}
                                            size="small"
                                            color="warning"
                                            icon={<ExclamationCircleOutlined />}
                                            label="Insufficient transaction fee"
                                        />
                                    </Tooltip>
                                )}
                            </Grid>
