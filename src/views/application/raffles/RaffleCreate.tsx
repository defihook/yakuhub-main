...
import { DEFAULT_PAY_TYPE, IMAGE_PROXY, RAFFLE_REWARD_TYPES, TICKETS_MAX, TOKEN_PAY_TYPE, WHITELIST_MAX } from 'config/config';
...
const RaffleCreate = () => {
    ...
    const updatePage = async () => {
        ...
        try {
            ...
            const { metadata } = nftData?.fetch;
            const item: NFTDetail = { ...metadata, ...(metadata.json || {}), mint };
            if (item) {
                setImage(item.image);
                setName(item.name);
