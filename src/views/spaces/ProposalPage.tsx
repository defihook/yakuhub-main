...
import { Box, Stack, Typography, Chip, CardContent, Grid, Divider, Avatar, Tooltip, Button } from '@mui/material';
...
const SpacePage = () => {
    ...
    const { data: space } = useAuthQuery(queries.GET_SPACE, { variables: { symbol }, fetchPolicy: 'cache-and-network' });
    const { data, loading } = useAuthQuery(queries.GET_PROPOSAL, { variables: { id }, fetchPolicy: 'cache-and-network' });

    const [DeleteProposal] = useAuthMutation(mutations.DELETE_PROPOSAL);
    const [CloseProposal] = useAuthMutation(mutations.CLOSE_PROPOSAL);
    const [VoteProposal] = useAuthMutation(mutations.VOTE_PROPOSAL);
                            ...
                            >
                                <CardContent sx={{ p: '0 !important', justifyContent: 'center' }}>
                                        ...
                                        <Box display="flex" justifyContent="flex-end" alignItems="center">
                                            {data.proposal.status ? (
                                                <>
                                                    ...
                                                    <Button size="small" variant="contained" color="primary" onClick={abstainVote}>
                                                        Abstain
                                                    </Button>

                                                    {publicKey?.toBase58() === data.proposal.author && (
                                                        <Button
                                                            size="small"
                                                            variant="contained"
                                                            color="warning"
                                                ...
                                                <>
                                                    <Button size="small" variant="contained" color="success" sx={{ mr: 1 }} disabled>
                                                        For
                                                    </Button>
                                                    <Button size="small" variant="contained" color="secondary" sx={{ mr: 1 }} disabled>
                                                        Against
                                                    </Button>
                                    ...
                                    <Box display="flex" flexDirection="row" justifyContent="flex-start" alignItems="center" sx={{ mb: 3 }}>
                                        ...
                                        <Chip
                                            size="small"
                                            color={data.proposal.status ? 'success' : 'primary'}
                                            label={<span>{data.proposal.status ? 'Open' : 'Closed'}</span>}
                                            sx={{ borderRadius: '40px !important', mr: 1 }}
                                        />
