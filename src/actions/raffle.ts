...
import { ComputeBudgetProgram, Connection, LAMPORTS_PER_SOL, PublicKey, Signer, SystemProgram, Transaction } from '@solana/web3.js';
...
import { getAssociatedTokenAccount, getATokenAccountsNeedCreate } from 'actions/shared';
 ...
 * @param whitelisted The variable if 2: winner get NFt as price and if 0: get whitelist spot  1: get spl token reward
...
export const createRaffle = async (
    ...
    let ix0 = await getATokenAccountsNeedCreate(connection, userAddress, globalAuthority, [nft_mint]);

    let ix1 = await getATokenAccountsNeedCreate(connection, userAddress, userAddress, [YAKU_TOKEN_MINT]);
    ...
    for (i = 10; i > 0; i--) {
        ...
        let state = await getStateByKey(connection, raffle);
        if (state === null) {
            console.log(i);
            break;
        }
 ...
 */
export const buyTicket = async (connection: Connection, wallet: WalletContextState, nft_mint: PublicKey, amount: number) => {
    ...
    const userAddress = wallet.publicKey;
    const [globalAuthority, bump] = await PublicKey.findProgramAddress([Buffer.from(GLOBAL_AUTHORITY_SEED)], program.programId);
    const raffleKey = await getRaffleKey(connection, nft_mint);
    if (raffleKey === null) return;
    let raffleState = await getRaffleState(connection, nft_mint);
 ...
 * @param nft_mint The nft_mint address
 */
export const revealWinner = async (connection: Connection, wallet: WalletContextState, raffleKey: PublicKey) => {
    if (wallet.publicKey === null) return;
    let cloneWindow: any = window;
    let provider = new anchor.AnchorProvider(connection, cloneWindow['solana'], anchor.AnchorProvider.defaultOptions());
    const program = new anchor.Program(IDL as anchor.Idl, RAFFLE_PROGRAM, provider);
 ...
 * @param nft_mint The nft_mint address
 */
export const claimReward = async (connection: Connection, wallet: WalletContextState, nft_mint: PublicKey) => {
    if (wallet.publicKey === null) return;
    let cloneWindow: any = window;
    let provider = new anchor.AnchorProvider(connection, cloneWindow['solana'], anchor.AnchorProvider.defaultOptions());
    const program = new anchor.Program(IDL as anchor.Idl, RAFFLE_PROGRAM, provider);
    ...
    let ix0 = await getATokenAccountsNeedCreate(connection, userAddress, userAddress, [nft_mint]);
 ...
 * @param nft_mint The nft_mint address
 */
export const withdrawNft = async (connection: Connection, wallet: WalletContextState, nft_mint: PublicKey) => {
    if (wallet.publicKey === null) return;
    let cloneWindow: any = window;
    let provider = new anchor.AnchorProvider(connection, cloneWindow['solana'], anchor.AnchorProvider.defaultOptions());
    const program = new anchor.Program(IDL as anchor.Idl, RAFFLE_PROGRAM, provider);
...
export const closeRaffle = async (connection: Connection, wallet: WalletContextState, raffleKey: PublicKey) => {
    if (wallet.publicKey === null) return;
    let cloneWindow: any = window;
    let provider = new anchor.AnchorProvider(connection, cloneWindow['solana'], anchor.AnchorProvider.defaultOptions());
    const program = new anchor.Program(IDL as anchor.Idl, RAFFLE_PROGRAM, provider);
...
export const getRaffleKey = async (connection: Connection, nft_mint: PublicKey): Promise<PublicKey | null> => {
    let cloneWindow: any = window;
    let provider = new anchor.AnchorProvider(connection, cloneWindow['solana'], anchor.AnchorProvider.defaultOptions());
    const program = new anchor.Program(IDL as anchor.Idl, RAFFLE_PROGRAM, provider);
    let poolAccounts = await connection.getParsedProgramAccounts(program.programId, {
        filters: [
            {
    ...
    if (poolAccounts.length !== 0) {
        ...
        let maxId = 0;
        for (let i = 0; i < len; i++) {
            let state = await getStateByKey(connection, poolAccounts[i].pubkey);
...
export const getRaffleGlobalState = async (connection: Connection) => {
    let cloneWindow: any = window;
    let provider = new anchor.AnchorProvider(connection, cloneWindow['solana'], anchor.AnchorProvider.defaultOptions());
    const program = new anchor.Program(IDL as anchor.Idl, RAFFLE_PROGRAM, provider);
    let poolAccounts = await connection.getParsedProgramAccounts(program.programId, {
...
export const getRaffleState = async (connection: Connection, nft_mint: PublicKey): Promise<RafflePool | null> => {
    let cloneWindow: any = window;
    let provider = new anchor.AnchorProvider(connection, cloneWindow['solana'], anchor.AnchorProvider.defaultOptions());
    const program = new anchor.Program(IDL as anchor.Idl, RAFFLE_PROGRAM, provider);
    let poolAccounts = await connection.getParsedProgramAccounts(program.programId, {
...
export const getStateByKey = async (connection: Connection, raffleKey: PublicKey): Promise<RafflePool | null> => {
    let cloneWindow: any = window;
    let provider = new anchor.AnchorProvider(connection, cloneWindow['solana'], anchor.AnchorProvider.defaultOptions());
    const program = new anchor.Program(IDL as anchor.Idl, RAFFLE_PROGRAM, provider);
