...
import { EPOCH, GLOBAL_AUTHORITY_SEED, METAPLEX, PROGRAM_ID, REWARD_TOKEN_DECIMAL, REWARD_TOKEN_MINT, USER_POOL_SIZE } from 'config/config';
import { getNftTokenAccount, getAssociatedTokenAccount, getATokenAccountsNeedCreate } from 'actions/shared';
...
export const setAmount = async (
    ...
    const provider = new anchor.AnchorProvider(connection, cloneWindow['solana'], anchor.AnchorProvider.defaultOptions());
...
export const calculateAllRewards = async (connection: Connection, wallet: WalletContextState) => {
    if (wallet.publicKey === null) return 0;

    const globalPool: GlobalPool | null = await getGlobalState(connection);
    if (globalPool === null) return 0;

    const userPool: UserPool | null = await getUserPoolState(connection, wallet);
...
export const calculateReward = async (connection: Connection, wallet: WalletContextState, nftMint: PublicKey) => {
    if (wallet.publicKey === null) return 0;
...
export const initUserPool = async (connection: Connection, wallet: WalletContextState) => {
    if (wallet.publicKey === null) return;

    let userAddress: PublicKey = wallet.publicKey;
    let cloneWindow: any = window;
    let provider = new anchor.AnchorProvider(connection, cloneWindow['solana'], anchor.AnchorProvider.defaultOptions());
    const program = new anchor.Program(IDL as anchor.Idl, PROGRAM_ID, provider);
...
export const stakeNft = async (
    ...
    let accountOfNft = await getNftTokenAccount(connection, mint);

    if (userTokenAccount.toBase58() !== accountOfNft.toBase58()) {
        let nftOwner = await getOwnerOfNft(connection, mint);
    ...
    let { instructions, destinationAccounts } = await getATokenAccountsNeedCreate(connection, userAddress, globalAuthority, [mint]);

    let userPoolKey = await PublicKey.createWithSeed(userAddress, 'user-pool', program.programId);

    let poolAccount = await connection.getAccountInfo(userPoolKey);
    if (poolAccount === null || poolAccount.data === null) {
        await initUserPool(connection, wallet);
    }
...
export const withdrawNft = async (connection: Connection, wallet: WalletContextState, mint: PublicKey) => {
    if (wallet.publicKey === null) return;
    const userAddress = wallet.publicKey;
    let cloneWindow: any = window;
    let provider = new anchor.AnchorProvider(connection, cloneWindow['solana'], anchor.AnchorProvider.defaultOptions());
...
export const withdrawAllNft = async (connection: Connection, wallet: WalletContextState, mintList: Array<PublicKey>) => {
    if (wallet.publicKey === null) return;
    const userAddress = wallet.publicKey;
    let cloneWindow: any = window;
    let provider = new anchor.AnchorProvider(connection, cloneWindow['solana'], anchor.AnchorProvider.defaultOptions());
    ...
    let { instructions, destinationAccounts } = await getATokenAccountsNeedCreate(connection, userAddress, globalAuthority, mintList);
    let userPoolKey = await PublicKey.createWithSeed(userAddress, 'user-pool', program.programId);
    ...
    const txId = await wallet.sendTransaction(txnMulti, connection);
...
export const claimRewardAll = async (connection: Connection, wallet: WalletContextState) => {
    if (wallet.publicKey === null) return;
    const userAddress = wallet.publicKey;
    let cloneWindow: any = window;
    let provider = new anchor.AnchorProvider(connection, cloneWindow['solana'], anchor.AnchorProvider.defaultOptions());
...
export const claimReward = async (connection: Connection, wallet: WalletContextState, mint: PublicKey) => {
    if (wallet.publicKey === null) return;
    const userAddress = wallet.publicKey;
    let cloneWindow: any = window;
    let provider = new anchor.AnchorProvider(connection, cloneWindow['solana'], anchor.AnchorProvider.defaultOptions());
...
export const getGlobalState = async (connection: Connection): Promise<GlobalPool | null> => {
    let cloneWindow: any = window;
    let provider = new anchor.AnchorProvider(connection, cloneWindow['solana'], anchor.AnchorProvider.defaultOptions());
    const program = new anchor.Program(IDL as anchor.Idl, PROGRAM_ID, provider);
...
export const getUserPoolState = async (connection: Connection, wallet: WalletContextState): Promise<UserPool | null> => {
...
const getOwnerOfNft = async (connection: Connection, nftMintPk: PublicKey): Promise<PublicKey> => {
